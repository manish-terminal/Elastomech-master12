...
const Inventory = () => {
  ...
  const handleEditToggle = async () => {
    if (isEditing) {
      ...
      await Promise.all(
        updates.map((item) =>
          fetch(`http://localhost:5001/api/items/${item._id}`, {
            method: "PUT",
  ...
  return (
    ...
    <div className="flex justify-center mb-6">
      ...
      >
        {isEditing ? <><FaSave className="inline-block mr-2" /> Save Changes</> : <><FaEdit className="inline-block mr-2" /> Edit Inventory</>}
      </button>
    </div>
    ...
    <motion.div
      ...
      transition={{ duration: 0.5 }}
    >
      <h3 className="text-2xl font-semibold text-gray-700 mb-4">Rubber</h3>
              ...
              <td className="py-3 px-4 border-b border-gray-300">
                {isEditing ? (
                  <input
                    type="number"
                    value={item.rate}
                    onChange={(e) => handleRateChange(index, "rubber", e.target.value)}
              ...
              <td className="py-3 px-4 border-b border-gray-300">
                {isEditing ? (
                  <input
                    type="number"
                    value={item.quantity}
                    onChange={(e) => handleQuantityChange(index, "rubber", e.target.value)}
                    className="w-full rounded-lg border border-gray-300 py-2 px-3 text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                ) : (
                  item.quantity
              ...
              <td className="py-3 px-4 border-b border-gray-300">
                {isEditing ? (
                  <input
                    type="number"
                    value={item.rate}
                    onChange={(e) => handleRateChange(index, "chemical", e.target.value)}
                    className="w-full rounded-lg border border-gray-300 py-2 px-3 text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                ) : (
                  item.rate
              ...
              <td className="py-3 px-4 border-b border-gray-300">
                {isEditing ? (
                  <input
                    type="number"
                    value={item.quantity}
                    onChange={(e) => handleQuantityChange(index, "chemical", e.target.value)}
                    className="w-full rounded-lg border border-gray-300 py-2 px-3 text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                ) : (
                  item.quantity
    ...
    >
      ...
      <form className="space-y-6">
        ...
        {/* Rate */}
        <div className="flex flex-col">
          <label htmlFor="rate" className="text-lg font-medium text-gray-700 mb-2">
            Rate per Kg (₹)
          </label>
          <input
            type="number"
        ...
        {/* Quantity */}
        <div className="flex flex-col">
          <label htmlFor="quantity" className="text-lg font-medium text-gray-700 mb-2">
